...
不要在render方法中使用HOC   
务必复制静态方法   
Refs不会被传递ref并不是一个prop-就像key一样，使用React.forwardRef来处理   
    
与第三方库协同：   
避免冲突的最简单方式是防止React组件更新，可以渲染无需更新的React元素  
```
...
React必须在作用域内    
当将字符串字面量赋值给prop时，值为未转义的，下面两个表达式是等价的
```
